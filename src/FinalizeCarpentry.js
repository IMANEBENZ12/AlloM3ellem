...
const Finalize = () => {
  ...
  useEffect(() => {
    const fetchMatchedHandymen = async () => {
      ...
      setIsLoading(true);
      try {
        const serviceType = bookingDetails?.serviceType || 'carpentry';
        const response = await fetch(`http://localhost:5000/api/handymen/match/${formData.zipCode}/carpentry`);
        ...
        // Filter handymen to include only those with carpentry skills
        const filteredHandymen = data.filter(handyman => handyman.skills.includes('carpentry'));
        ...
        {selectedHandyman && (
          <div className="booking-summary-card">
            ...
            <div className="summary-details">
              <p><strong>{t('service')}:</strong> {bookingDetails?.serviceType || t('CarpentryService')}</p>

...
const Finalize = () => {
  ...
  useEffect(() => {
    const fetchMatchedHandymen = async () => {
      ...
      setIsLoading(true);
      try {
        const serviceType = bookingDetails?.serviceType || 'plumbing'; // Default to plumbing service
        const response = await fetch(`http://localhost:5000/api/handymen/match/${formData.zipCode}/plumbing`);
        ...
        // Filter handymen to include only those with plumbing skills
        const filteredHandymen = data.filter(handyman => handyman.skills.includes('plumbing'));
        ...
        {selectedHandyman && (
          <div className="booking-summary-card">
            ...
            <div className="summary-details">
              <p><strong>{t('service')}:</strong> {bookingDetails?.serviceType || t('plumbingService')}</p>
